...
**CRITICAL: Never merge automatically.** After completing all checks (CI, comments), always stop and ask the user for explicit confirmation before merging. 
The user may have additional instructions, want to review something first, or need to coordinate timing.
...
**If the current branch has an open PR, always reuse it.** 
Push new commits to the same branch — do NOT create a new branch or a new PR. 
The existing PR will update automatically.

**IMPORTANT:** There should normally be only one open PR at a time. 
If multiple PRs are open, ask the user which one to merge first. 
Do NOT create new PRs until existing ones are merged.
...
- If there are **unresolved comments or change requests**, stop. 
  Show the comments to the user and address them. After fixing, push changes and go back to Step 2.
- If there are **no comments** or all are resolved/approved, proceed.

### Step 3.5: Resolve merge conflicts

Check if the PR has merge conflicts and resolve them
