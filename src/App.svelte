<script>
  ...
  let gpuInfo = $state(null);       // { vendor, architecture, device, description, features, limits }
  let showGpuPanel = $state(false);
  let generationPhase = $state(null); // null | "preparing" | "generating"
  ...
  function send() {
    ...
    tps = null;
    isRunning = true;
    // Strip Svelte $state proxy and omit thinking from history sent to model
  ...
  /** More precise for progress: 2 decimals for MB/GB, exact for B/KB */
      ...
      {#each messages as msg, i}
        <div class="bubble {msg.role}">
          ...
          {#if msg.role === "assistant"}
            <!-- Thinking toggle / live indicator -->
            ...
            <!-- Collapsible thinking content (after thinking is done) -->
            {#if msg.thinking}
