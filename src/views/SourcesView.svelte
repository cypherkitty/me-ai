<script>
  ...
  // ── Source metadata ────────────────────────────────────────────────
  const SOURCE_META = {
    gmail:     { color: "#ea4335", icon: "M", label: "Gmail",     platform: "Email",     live: true  },
    telegram:  { color: "#26a5e4", icon: "T", label: "Telegram",  platform: "Messenger", live: false },
    instagram: { color: "#e1306c", icon: "I", label: "Instagram", platform: "Social",    live: false },
    youtube:   { color: "#ff0000", icon: "Y", label: "YouTube",   platform: "Video",     live: false },
    slack:     { color: "#611f69", icon: "S", label: "Slack",     platform: "Messenger", live: false },
    twitter:   { color: "#1da1f2", icon: "X", label: "Twitter/X", platform: "Social",    live: false },
  };

  // All sources in display order (live first, then coming-soon alphabetical)
  const ALL_SOURCES = ["gmail", "telegram", "instagram", "slack", "twitter", "youtube"];
    ...
    if (clientId && !authInitialized) {
      initGoogleAuth(clientId)
        .then(async () => {
          ...
          if (saved) { accessToken = saved.access_token; scheduleTokenRefresh(); fetchProfile(); }
        })
  ...
  function scheduleTokenRefresh() {
    ...
    refreshTimer = setTimeout(async () => {
      try { const r = await refreshToken(); accessToken = r.access_token; scheduleTokenRefresh(); }
      catch { accessToken = null; profile = null; gmailError = "Session expired. Please sign in again."; }
    }, delay);
  }

  async function ensureValidToken() {
    if (isTokenValid()) return accessToken;
    try { const r = await refreshToken(); accessToken = r.access_token; scheduleTokenRefresh(); return accessToken; }
    catch { accessToken = null; profile = null; gmailError = "Session expired. Please sign in again."; return null; }
  }
  ...
  async function saveClientId() {
    ...
    clientId = t; gmailError = null; showClientIdEdit = false;
  }

  async function signIn() {
    gmailError = null; loadingAuth = true;
    try {
      ...
      await fetchProfile();
    } catch (e) { gmailError = e.message; }
    finally { loadingAuth = false; }
  }
  ...
  async function loadLocalMessages(append = false) {
    ...
    } catch (e) { gmailError = `Failed to load messages: ${e.message}`; }
  ...
  async function startSync(limit) {
    ...
    try {
      ...
      await refreshSyncStatus(); await loadLocalMessages(false);
    } catch (e) { if (e.name !== "AbortError" && accessToken) gmailError = `Sync failed: ${e.message}`; }
  ...
  async function startSyncMore(limit) {
    ...
    try {
      ...
      await refreshSyncStatus(); await loadLocalMessages(false);
    } catch (e) { if (e.name !== "AbortError" && accessToken) gmailError = `Sync more failed: ${e.message}`; }
    finally { isSyncing = false; }
  }

  async function handleClearData() {
    try {
      await clearGmailData(); await refreshSyncStatus();
      emailMessages = []; totalLocalMessages = 0; localOffset = 0; showClearConfirm = false;
    } catch (e) { gmailError = `Failed to clear data: ${e.message}`; }
  }
...
<div class="flex h-full overflow-hidden">
  ...
  <div class="w-56 shrink-0 flex flex-col border-r border-border bg-sidebar overflow-hidden">
    <div class="px-3 pt-4 pb-2 shrink-0">
      <p class="text-[0.6rem] font-semibold uppercase tracking-widest text-muted-foreground/50">Sources</p>
    </div>
        ...
        {#each ALL_SOURCES as name}
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <p class={cn("text-sm font-medium truncate", isSelected && isLive ? "text-foreground" : "text-foreground/80")}>
                {meta.label}
              </p>
              <p class="text-[0.65rem] text-muted-foreground/50 truncate">{meta.platform}</p>
            </div>
  ...
  <div class="flex-1 overflow-hidden flex flex-col bg-background">
    ...
    {#if selectedSource === "gmail"}
      ...
      {#if isSignedIn}
        ...
        <div class="flex items-center gap-3 px-4 py-2.5 border-b border-border shrink-0 bg-sidebar/40">
          <!-- Avatar + email -->
          <div class="flex items-center gap-2 min-w-0">
            <div class="size-7 rounded-full bg-primary/15 flex items-center justify-center text-xs font-bold text-primary shrink-0">
          ...
          <div class="flex items-center gap-2 ml-auto shrink-0">
            ...
            {#if isSyncing && syncProgress}
              <span class="text-xs text-muted-foreground/60">{syncProgress.message || "Syncing…"}</span>
            {:else if syncStatus?.synced}
              <span class="text-xs text-muted-foreground/40">{syncStatus.totalItems.toLocaleString()} emails · {formatTimeAgo(syncStatus.lastSyncAt)}</span>
            {/if}
            ...
            >
              {#each LIMIT_OPTIONS as opt}<option value={opt}>{opt}</option>{/each}
            </select>
            ...
            {#if syncStatus?.synced && syncStatus.hasMore}
              <Button variant="outline" size="sm" onclick={() => startSyncMore(syncLimit)} disabled={isSyncing} class="h-7 text-xs">
                More
              </Button>
            {/if}

            <!-- Sign out -->
            <button onclick={signOut} class="text-muted-foreground/30 hover:text-destructive transition-colors p-1 rounded" title="Sign out">
        ...
        <!-- Error banner -->
        {#if gmailError}
          <div class="mx-4 mt-2 px-3 py-2 rounded border border-destructive/30 bg-destructive/8 text-xs text-destructive flex items-center justify-between shrink-0">
            <span>{gmailError}</span>
            <button onclick={() => gmailError = null} class="ml-2 opacity-60 hover:opacity-100">✕</button>
          </div>
        {/if}
        ...
        <div class="flex items-center gap-2 px-4 py-2.5 border-b border-border shrink-0">
          ...
          <Button onclick={searchLocal} disabled={loadingMessages} variant="outline" size="sm" class="shrink-0 h-8">
            ...
            {:else}
              <div class="flex flex-col items-center justify-center gap-3 py-20 text-center">
                ...
                <p class="text-sm text-muted-foreground/40">
                  {searchQuery ? `No emails match "${searchQuery}"` : "No emails synced yet — click Download above"}
                </p>
              </div>
            {/if}
          </div>
          ...
          <div class="px-4 py-2 border-t border-border shrink-0 flex items-center justify-end">
            {#if showClearConfirm}
              <div class="flex items-center gap-2">
                ...
                <Button variant="destructive" size="sm" onclick={handleClearData} class="h-6 text-xs px-2">Delete</Button>
                <Button variant="outline" size="sm" onclick={() => showClearConfirm = false} class="h-6 text-xs px-2">Cancel</Button>
              </div>
            {:else}
              <button onclick={() => showClearConfirm = true} class="flex items-center gap-1 text-[0.65rem] text-muted-foreground/25 hover:text-destructive transition-colors">
      ...
      {:else}
        ...
        <div class="flex flex-col items-center justify-center h-full gap-5 px-8">
          ...
          {#if gmailError}
            <p class="text-sm text-destructive text-center max-w-xs">{gmailError}</p>
          {/if}
          ...
          <div class="flex flex-col items-center gap-2 w-full max-w-[260px]">
            ...
            >
              {#if loadingAuth}
                <div class="size-4 border-2 border-zinc-300 border-t-zinc-700 rounded-full animate-spin shrink-0"></div>
                Signing in…
              {:else}
            ...
            <a href="#oauth-redirect" class="text-xs text-muted-foreground/40 hover:text-primary transition-colors">
          ...
          <div class="w-full max-w-[260px]">
            {#if !showClientIdEdit}
              <div class="flex items-center gap-1.5 text-[0.65rem] text-muted-foreground/30">
                <span>Client ID: {isDefaultClientId ? "shared default" : clientId.slice(0, 16) + "…"}</span>
                <button onclick={() => showClientIdEdit = true} class="text-primary hover:underline ml-auto">Change</button>
              </div>
            {:else}
              <div class="flex flex-col gap-1.5">
                <Input bind:value={clientIdInput} placeholder="Paste your Client ID…" class="h-8 text-xs font-mono" />
                <div class="flex gap-1.5">
                  <Button size="sm" onclick={saveClientId} class="flex-1 h-7 text-xs">Save</Button>
                  <Button variant="outline" size="sm" onclick={() => showClientIdEdit = false} class="flex-1 h-7 text-xs">Cancel</Button>
                </div>
              </div>
            {/if}
          </div>
    ...
    {:else}
      ...
      <div class="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
        ...
        <div>
          <p class="text-base font-semibold text-foreground mb-1">{meta?.label ?? selectedSource}</p>
