...
use crate::db::{index_get_all, key_range_only, store_get, store_get_all};
...
pub async fn get_sources() -> Result<Vec<SourceRow>, CoreError> {
    let mut rows: Vec<SourceRow> = store_get_all(store::SM_SOURCES, None, None).await?;
...
pub async fn get_actions() -> Result<Vec<ActionRow>, CoreError> {
    let mut rows: Vec<ActionRow> = store_get_all(store::SM_ACTIONS, None, None).await?;
    rows.sort_by(|a, b| a.name.cmp(&b.name));
    Ok(rows)
}

/// Fetch plugins with nested actions and sources (for rules UI).
pub async fn get_plugins() -> Result<Vec<serde_json::Value>, CoreError> {
    let plugins: Vec<PluginRow> = store_get_all(store::SM_PLUGINS, None, None).await?;
    let mut out = Vec::with_capacity(plugins.len());
    for p in plugins {
        let name = p.name.clone();
        let range_act = key_range_only(&name)?;
        let actions: Vec<PluginActionRow> = index_get_all(
        ...
        for an in &action_names {
            if let Ok(Some(a)) = store_get::<ActionRow>(store::SM_ACTIONS, an).await {
                action_labels.push(serde_json::json!({ "name": a.name, "label": a.label }));
            }
        }
        let range_src = key_range_only(&name)?;
        let sources: Vec<PluginSourceRow> = index_get_all(
        ...
        for sn in &source_names {
            if let Ok(Some(s)) = store_get::<SourceRow>(store::SM_SOURCES, sn).await {
