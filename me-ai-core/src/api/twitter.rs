...
fn to_js_val(v: serde_json::Value) -> Result<JsValue, CoreError> {
    serde_wasm_bindgen::to_value(&v).map_err(|e| CoreError::Serialize(e.to_string()))
}

pub async fn get_me(token: &str) -> Result<JsValue, CoreError> {
    to_js_val(
        twitter_get(
            token,
...
pub async fn get_user_timeline(
    ...
    to_js_val(twitter_get(token, &path).await?)
}
...
pub async fn search_recent_tweets(
    ...
    to_js_val(resp.json().await.map_err(|e| CoreError::Plugin(e.to_string()))?)
}

pub async fn get_tweet(token: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(
        twitter_get(
            token,
            &format!(
...
pub async fn like_tweet(token: &str, user_id: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(
        twitter_post(
            token,
...
pub async fn unlike_tweet(token: &str, user_id: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(twitter_delete(token, &format!("/users/{user_id}/likes/{tweet_id}")).await?)
}

pub async fn retweet(token: &str, user_id: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(
        twitter_post(
            token,
...
pub async fn unretweet(token: &str, user_id: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(twitter_delete(token, &format!("/users/{user_id}/retweets/{tweet_id}")).await?)
}

pub async fn bookmark_tweet(token: &str, user_id: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(
        twitter_post(
            token,
...
pub async fn remove_bookmark(token: &str, user_id: &str, tweet_id: &str) -> Result<JsValue, CoreError> {
    to_js_val(twitter_delete(token, &format!("/users/{user_id}/bookmarks/{tweet_id}")).await?)
}
