...
describe("emailToJson", () => {
  ...
  it("preserves all Gmail API fields", () => {
    ...
    expect(j.threadId).toBe("thread456");
    expect(j.labelIds).toEqual(["INBOX", "UNREAD"]);
    expect(j.snippet).toBe("Hi Bob, How are you?");
    expect((j.payload as { headers: unknown[] }).headers).toHaveLength(4);
  });
...
describe("emailToJsonString", () => {
  it("returns valid JSON matching the raw Gmail response", () => {
    ...
    expect((parsed.payload as { headers: { name: string }[] }).headers[0].name).toBe("From");
  });
