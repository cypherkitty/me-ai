...
class GmailPlugin extends BasePlugin {
  ...
  extractMessageId(ctx: PluginContext): string | null {
    const data = ctx.event?.data as { emailId?: string; id?: string } | undefined;
    const id = data?.emailId ?? data?.id;
    if (!id) return null;
    return typeof id === "string" && id.startsWith("gmail:") ? id.substring(6) : String(id);
  }
  ...
  registerAllHandlers(): void {
    ...
    this.registerHandler({
      ...
      execute: async (ctx) => {
        const messageId = this.extractMessageId(ctx);
        const labelId = ctx.config?.labelId as string | undefined;
        if (!messageId) throw new Error("No message ID provided");
