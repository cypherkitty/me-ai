<script lang="ts">
  ...
  const PLUGIN_META: Record<string, { color: string; icon: string; desc: string }> = {
    ...
    ai_classifier: {
      color: "#10b981",
      icon: "C",
      desc: "AI event classification (runs first on all events)",
    },
  };
  ...
  async function load() {
    loading = true;
    try {
      plugins = (await getCore().getPlugins()) as unknown as PluginItem[];
    } catch (e) {
      console.error("PluginsView load error:", e);
    }
    loading = false;
  }
...
<div class="flex flex-col h-full overflow-hidden">
  <div class="px-8 pt-5 pb-4 shrink-0 border-b border-border">
    ...
    <p class="text-xs text-muted-foreground">
      {plugins.filter((p) => p.enabled).length} active — plugins provide actions and handle sources.
    </p>
  </div>
  ...
  <ScrollArea class="flex-1 px-8 pb-6">
    ...
    {:else}
      <div class="flex flex-col gap-3">
              ...
              <div class="flex items-start gap-2.5">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-baseline gap-1.5 mb-0.5">
                    ...
                    <code class="text-[0.62rem] text-muted-foreground/50 font-mono"
                      >v{plugin.version}</code
                    >
                  </div>
        ...
        <!-- Info box -->
        <div
          class="flex items-start gap-2.5 px-3.5 py-3 rounded border border-border/40 bg-muted/20 text-muted-foreground mt-1"
        >
          ...
          <p class="text-xs leading-relaxed">
            ...
            The <strong class="text-muted-foreground/80">AI Classifier</strong> plugin is always active;
            it runs before all rules and assigns EventType + EventCategory to every incoming event.
          </p>
        </div>
      </div>
    {/if}
