<script lang="ts">
  ...
  let isPreparingModel = $derived(/compiling shaders|warming up model/i.test(message));
  ...
  $effect(() => {
    ...
    if (lastSampleLoaded === null || lastSampleAt === null || aggregateLoaded < lastSampleLoaded) {
  ...
  <div class="max-w-[560px] w-full mx-auto p-8 flex flex-col items-center text-center gap-4">
    ...
    {#if items.length > 0}
      <Card
        class={cn(
          ...
          isPreparingModel ? "border-success/30 bg-success/[0.04]" : "border-primary/15 bg-card"
        )}
      >
    ...
    {#if activeItems.length > 0}
      <div class="w-full flex items-center justify-between px-1">
        <span
          class="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground/55"
        >
          Active downloads
        </span>
      ...
      <Card class="w-full transition-colors">
        <CardContent class="pt-4 pb-3 px-4">
          ...
          {#if item.total}
            ...
            <div
              class="flex items-baseline gap-1.5 flex-wrap text-xs text-muted-foreground tabular-nums"
            >
              ...
              <span class="text-[0.62rem] opacity-35 ml-1"
                >({(item.loaded || 0).toLocaleString()} / {item.total.toLocaleString()} B)</span
              >
            </div>
          {:else}
            <div class="h-1 w-full bg-muted rounded-full overflow-hidden mb-2">
              <div
                class="h-full w-[30%] bg-primary rounded-full animate-[slide_1.5s_ease-in-out_infinite]"
              ></div>
            </div>
    ...
    {#if completedItems.length > 0}
      <details class="w-full group">
        <summary
          class="flex items-center justify-between gap-3 px-4 py-3 rounded-xl border border-success/20 bg-success/[0.035] text-left cursor-pointer list-none hover:bg-success/[0.05] transition-colors"
        >
            ...
            <Card class="w-full transition-colors border-success/25 bg-success/[0.04]">
              <CardContent class="pt-4 pb-3 px-4">
                ...
                {#if item.total}
                  <Progress
                    value={pct ?? 0}
                    class="h-1 mb-2 [&_[data-slot=progress-indicator]]:bg-[var(--color-success)]"
                  />
                  <div
                    class="flex items-baseline gap-1.5 flex-wrap text-xs text-muted-foreground tabular-nums"
                  >
                    <span class="text-foreground font-medium"
                      >{formatBytesPrecise(item.loaded || 0)}</span
                    >
                    <span class="opacity-30">/</span>
                    <span class="opacity-60">{formatBytesPrecise(item.total)}</span>
                    <span class="text-[0.62rem] opacity-35 ml-1"
