<script lang="ts">
  ...
  const CATEGORY_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    NOISE: {
      bg: "color-mix(in srgb, #9ca3af 10%, transparent)",
      text: "#9ca3af",
      border: "color-mix(in srgb, #9ca3af 20%, transparent)",
    },
    INFO: {
      bg: "color-mix(in srgb, #60a5fa 10%, transparent)",
      text: "#60a5fa",
      border: "color-mix(in srgb, #60a5fa 20%, transparent)",
    },
    CRITICAL: {
    ...
    COMMAND: {
      ...
      text: "#fbbf24",
      border: "color-mix(in srgb, #fbbf24 20%, transparent)",
    },
  };
...
>
  ...
  >
    ...
    <span class="flex-1 text-[0.82rem] font-medium text-foreground/85 truncate tracking-tight"
      >{msg.title}</span
    >
    ...
    <svg
      class={cn(
        "size-3 text-muted-foreground/30 shrink-0 transition-transform",
        cardOpen && "rotate-180"
      )}
      viewBox="0 0 24 24"
      fill="none"
  ...
  {#if cardOpen}
    {#if msg.description}
      <div
        class="px-3.5 py-2.5 text-[0.8rem] text-muted-foreground leading-relaxed border-t border-border whitespace-pre-wrap"
      >
        {msg.description}
      </div>
    {/if}
      ...
      <ScrollArea class="border-t border-border h-[min(320px,50vh)]">
        <div class="pr-2">
          {#each msg.steps as step (step.id)}
            ...
            {@const hasBody =
              step.expandable && (step.subContent || step.thumbnail || step.badges?.length)}
            ...
            <div class="border-b border-border last:border-b-0">
              ...
              {#snippet stepRow()}
                ...
                <span
                  class={cn(
                    ...
                    stepIconClass(step.status)
                  )}
                >
                  {#if step.status === "running"}
                    <span
                      class="size-2.5 rounded-full border border-border border-t-primary animate-spin"
                    ></span>
                ...
                <span
                  class={cn("flex-1 min-w-0 truncate text-[0.78rem]", stepTextClass(step.status))}
                >
                  {step.label}
                </span>
                  ...
                  <span class="flex gap-1 shrink-0">
                    {#each step.badges as badge (badge)}
                      ...
                      <span
                        ...
                        style:color={gs.text}
                        style:border-color={gs.border}>{badge}</span
                      >
                    {/each}
                  </span>
                {/if}
                ...
                {#if step.detail || (step.startedAt && step.status === "running")}
                  <span
                    class={cn("text-[0.68rem] tabular-nums shrink-0", stepTextClass(step.status))}
                  >
                    {#if step.detail}{step.detail}{/if}
                    {#if step.startedAt && step.status === "running"}
                      · {elapsed(step.startedAt)}{/if}
                  </span>
                {/if}
                ...
                {#if hasBody}
                  <svg
                    class={cn(
                      "size-2.5 text-muted-foreground/30 shrink-0 transition-transform",
              ...
              {#if hasBody && isExpanded}
                <div
                  class="px-3.5 pb-2.5 pt-1 pl-9 border-t border-border bg-foreground/[0.01] flex flex-col gap-2"
                >
                  {#if step.thumbnail}
                    <img
                      src={step.thumbnail}
                      alt="screenshot"
                      class="max-w-[260px] w-full rounded border border-border"
                    />
                  {/if}
                  {#if step.badges?.length}
                  ...
                  {#if step.subContent}
                    <pre
                      class="font-[inherit] text-[0.74rem] text-muted-foreground leading-relaxed whitespace-pre-wrap break-words m-0">{step.subContent}</pre>
                  {/if}
                </div>
              {/if}
            </div>
