...
<div
  class="flex flex-col gap-1.5 mb-5 px-4 py-3.5 rounded border-l-4 border border-destructive/20 border-l-destructive bg-destructive/5"
>
  {#if ondismiss}
  ...
  {#if parsed.link}
    <a
      ...
      target="_blank"
      rel="noopener"
      class="text-[0.8rem] font-medium text-primary hover:underline">{parsed.link.label} →</a
    >
  {/if}
