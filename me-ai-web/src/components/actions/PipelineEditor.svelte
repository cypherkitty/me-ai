    ...
    untrack(() => {
      if (isOpen && currentRule) {
        ...
        commands = (currentRule.actions || []).map((c) => {
          ...
          if (!cloned.id) {
            cloned.id = (cloned.commandId || "cmd") + "_" + Math.random().toString(36).substring(7);
          }
          ...
          if (
            ...
            cloned.commandId = cloned.commandId.slice(cloned.pluginId.length + 1);
          }
          // Try to enrich name/description from PLUGIN_ACTIONS registry
          if (!cloned.name || !cloned.description) {
            const pluginGroup = PLUGIN_ACTIONS.find((p) => p.pluginId === cloned.pluginId);
            const handler = pluginGroup?.actions.find((a) => a.actionId === cloned.commandId);
            if (handler) {
              cloned.name = cloned.name || handler.name;
              cloned.description = cloned.description || handler.description;
              cloned.icon = cloned.icon || ACTION_ICONS_MAP[handler.actionId];
            } else {
              // Fallback: at minimum show the commandId so the card isn't blank
              cloned.name = cloned.name || cloned.commandId || "Unknown action";
              cloned.description =
                cloned.description ||
                (cloned.pluginId ? `${cloned.pluginId} · ${cloned.commandId}` : "");
            }
          }
          return cloned;
        });
  ...
  <div class="editor-overlay" role="dialog">
    <div class="editor-panel">
      ...
      <div class="editor-header">
        <div class="header-left border-r border-border pr-4 mr-4 shrink-0">
          <h2>Unified Pipeline Editor</h2>
          <span class="subtitle font-mono text-muted-foreground">{rule?.id?.slice(0, 8)}</span>
        </div>
        ...
        <div class="header-right shrink-0 flex items-center gap-3">
          <button class="small-btn primary" onclick={savePipeline}>Save Pipeline</button>
      ...
      <div class="editor-body p-8 flex flex-col items-center bg-black">
        {#if rule && rule.triggers}
          <div class="flex items-stretch justify-center max-w-5xl w-full gap-5 mx-auto mt-8">
            <!-- TRIGGER CONTAINER -->
            <div class="w-[340px] shrink-0 flex flex-col items-center z-10">
              <div
                ...
                >
                  ...
                  <div class="flex flex-col">
                    <span
                      ...
                      >Trigger</span
                    >
                    <span class="text-base font-semibold">Detect AI Event Type</span>
                  </div>
                </div>
                <div class="p-5 bg-background border-t border-border/50">
                  <div class="mb-3">
                    {#if triggers.length > 0 && triggers[0].type === "event_category"}
                      <p class="text-[11px] text-muted-foreground/60 leading-relaxed mb-3">
                        Event types linked to this category. AI-classified emails matching these
                        types will trigger the pipeline.
                      </p>
                      ...
                      {#if (rule?._eventTypes || []).filter((et) => !typesToDelete.includes(et.name)).length > 0 || typesToMove.length > 0}
                        <div class="flex flex-wrap gap-1.5 mb-3">
                          {#each (rule?._eventTypes || []).filter((et) => !typesToDelete.includes(et.name)) as et (et.name)}
                            ...
                            >
                              <span class="size-1.5 rounded-full bg-blue-500/60 shrink-0"></span>
                              {et.name}
                              <button
                                class="ml-0.5 text-blue-500/30 hover:text-red-400 transition-colors text-[10px] font-bold"
                                onclick={() => {
                                  typesToDelete = [...typesToDelete, et.name];
                                  typesToMove = typesToMove.filter((t) => t !== et.name);
                                }}
                                aria-label="Remove {et.name} from category">✕</button
                              >
                            </div>
                          {/each}
                          {#each typesToMove as et (et)}
                            ...
                            >
                              <span class="size-1.5 rounded-full bg-emerald-500/60 shrink-0"></span>
                              {et}
                              <span class="text-[9px] text-emerald-500/50 uppercase font-bold"
                                >new</span
                              >
                              <button
                                class="ml-0.5 text-emerald-500/30 hover:text-red-400 transition-colors text-[10px] font-bold"
                                onclick={() => (typesToMove = typesToMove.filter((t) => t !== et))}
                                aria-label="Undo adding {et}">✕</button
                              >
                            </div>
                          {/each}
                        </div>
                      ...
                      <select
                        onchange={(e) => {
                          ...
                          const val = sel.value;
                          if (val && !typesToMove.includes(val)) {
                            typesToMove = [...typesToMove, val];
                            typesToDelete = typesToDelete.filter((t) => t !== val);
                          }
                          sel.selectedIndex = 0;
                        }}
                      ...
                      >
                        <option value="" disabled selected>+ Add event type…</option>
                        {#each eventTypes.filter((et) => {
                          ...
                          <option value={et}
                            >{et
                              .toLowerCase()
                              .replace(/_/g, " ")
                              .replace(/\b([a-z])/g, (c) => c.toUpperCase())}</option
                          >
                        {/each}
                      </select>
                    {:else if triggers.length > 0}
                      <p class="text-xs text-muted-foreground leading-relaxed mb-3">
                        Trigger this pipeline when the AI engine identifies the email's core purpose
                        as this Event Type:
                      </p>
                      <select
            ...
            <!-- CONNECTOR -->
            <div class="flex flex-col items-center w-12 pt-[64px] shrink-0">
              <div class="w-full h-px bg-border flex items-center justify-center">
                <div
              ...
              >
                ...
                >
                  <svg
                    ...
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="size-5 fill-emerald-500/20"><polygon points="6 3 20 12 6 21 6 3" /></svg
                  >
                </div>
                <div class="flex flex-col">
                  <span
                    ...
                    >Actions</span
                  >
                  <span class="text-base font-semibold">Execute in order</span>
                </div>
              </div>
              <div class="p-5 bg-background flex-1 flex flex-col gap-3 min-h-[300px]">
                ...
                >
                  <span class="text-xl leading-none mb-0.5">+</span> Add Action
                </button>
              </div>
            </div>
          </div>
      ...
      {#if showActionPicker}
        ...
        >
          <div class="flex items-center justify-between px-6 py-4 border-b border-border/50">
            <h3 class="text-base font-semibold text-foreground">Select Action</h3>
            <button class="close-btn-small" onclick={() => (showActionPicker = false)}
              >&#10005;</button
            >
          </div>
                  ...
                  {#each group.actions as handler (handler.actionId)}
                    ...
                    >
                      <span
                        ...
                        >{ACTION_ICONS_MAP[handler.actionId] ?? "·"}</span
                      >
                      <span class="text-xs font-semibold text-foreground/80">{handler.name}</span>
                      {#if added}<span
        ...
        >
          <div class="flex items-center justify-between px-6 py-4 border-b border-border/50">
            <h3 class="text-base font-semibold flex items-center gap-2">
              <span class="text-xl">{editingCmd.icon || "⚙️"}</span>
              {editingCmd.name || "Edit Action"}
            </h3>
            <button class="close-btn-small" onclick={cancelEdit}>&#10005;</button>
          </div>
          <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
            <div class="flex flex-col gap-4">
              <div class="flex gap-4">
                <div class="flex flex-col gap-2 w-16 shrink-0">
                  <label
                    for="edit-icon"
                    class="text-xs font-bold text-muted-foreground/80 uppercase">Icon</label
                  >
                  <input
                    id="edit-icon"
                    type="text"
                ...
                <div class="flex flex-col gap-2 flex-1">
                  <label
                    for="edit-name"
                    class="text-xs font-bold text-muted-foreground/80 uppercase">Name</label
                  >
                  <input
                    id="edit-name"
                    type="text"
              ...
              <div class="flex flex-col gap-2">
                <label for="edit-desc" class="text-xs font-bold text-muted-foreground/80 uppercase"
                  >Description</label
                >
                <input
                  id="edit-desc"
              ...
              <div class="flex flex-col gap-2 mt-4 pt-4 border-t border-border/50">
                <label
                  for="edit-handler"
                  class="text-xs font-bold text-muted-foreground/80 uppercase">Plugin Handler</label
                >
                <select
                  id="edit-handler"
                ...
                >
                  <option value="" disabled>Select handler function...</option>
                  {#each PLUGIN_ACTIONS as group (group.pluginId)}
                    <optgroup label={group.pluginName}>
                      {#each group.actions as handler (handler.actionId)}
                        <option value="{group.pluginId}:{handler.actionId}">{handler.name}</option>
                      {/each}
                    </optgroup>
                  {/each}
                </select>
          ...
          <div class="p-5 border-t border-border/50 flex gap-3 bg-card/50">
            <button
