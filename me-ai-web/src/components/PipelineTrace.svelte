...
{#if steps?.length}
  <div class={cn("rounded bg-trace-bg border border-border px-4 py-3", className)}>
    <p class="text-[0.62rem] font-bold uppercase tracking-widest text-muted-foreground mb-3">
      Execution Trace
    </p>
    <div class="flex flex-col relative">
      {#each steps as step, i (i)}
        {@const isSuccess = step.success === true || step.status === "done"}
        {@const isError = step.success === false || step.status === "error"}
        {@const isRunning = step.status === "running"}
        {@const isPending = step.status === "pending" || (!isSuccess && !isError && !isRunning)}
        {@const name = step.actionName || step.commandId || step.label}
        {@const msg =
          step.message || step.error || (step.status === "error" ? "Execution failed" : "")}

        <div class="flex items-center gap-3 mt-1 mb-4 relative last:mb-0">
          {#if i < steps.length - 1}
            <div class="absolute left-[13px] top-7 w-px h-7 bg-border"></div>
          {/if}
          ...
          <div
            class={cn(
              ...
              isRunning && "bg-primary border-2 border-primary/30 animate-pulse",
          ...
          >
            ...
            {:else if isRunning}
              <div class="size-1.5 rounded-full bg-white animate-ping"></div>
            {:else}
          ...
          <div class="flex flex-col min-w-0">
            <span class="text-sm text-muted-foreground flex items-center gap-1.5 flex-wrap">
              <span class={cn("opacity-70", isRunning && "text-primary font-medium opacity-100")}>
